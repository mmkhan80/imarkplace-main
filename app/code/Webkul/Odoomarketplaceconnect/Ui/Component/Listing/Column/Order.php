        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                if (isset($item['magento_id'])) {
                    ...
                    // $item['magento_id'] = '<a href="'.$this->urlBuilder->getUrl(self::ORDER_PATH_VIEW, ['order_id' => $item['magento_id']]).'" target="_blank">'.$name.'</a>';
                    $item['order_product'] = '<a href="'.$this->urlBuilder->getUrl(self::PRODUCT_PATH_VIEW, ['id' => $productId]).'" target="_blank">'.$proName.'</a>';
                }
            }
        }
        return $dataSource;
